...
func (f *progress) Passed(step *gherkin.Step, match *godog.StepDef) {
	f.Lock()
	defer f.Unlock()
...
func (f *progress) Skipped(step *gherkin.Step, match *godog.StepDef) {
	f.Lock()
	defer f.Unlock()
...
func (f *progress) Undefined(step *gherkin.Step, match *godog.StepDef) {
	f.Lock()
	defer f.Unlock()
...
func (f *progress) Failed(step *gherkin.Step, match *godog.StepDef, err error) {
	f.Lock()
	defer f.Unlock()
...
func (f *progress) Pending(step *gherkin.Step, match *godog.StepDef) {
	f.Lock()
	defer f.Unlock()
